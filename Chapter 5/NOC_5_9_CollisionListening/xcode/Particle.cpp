...
void Particle::makeBody() {
    ...
    //set this Particle object in the body's user data
...
void Particle::draw() {    
    ...
    if ( mContacted ) {
        // we have contact with some other ball:
        ci::gl::color( 1, .6, .6, 1 );
    } else {
        ci::gl::color( .6, .6, .6, 0.6 );
    }
...
void Particle::startContact() {  mContacted = TRUE; }

void Particle::endContact() {
    // do nothing when contact ends
//   mContacted = FALSE;
}

void Particle::change() { mColor = ci::ColorA(.9, .6, .6, 1.); }
